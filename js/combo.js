$(document).ready(function(){ //código a ejecutar cuando está listo para recibir instrucciones.
	...
	//Evento del Boton de Consultar todas la compras Proovedores 

   	$("#ConsultarProvedorTodo").click(function () { 
   		  $(".error").remove();
        if( $("#dtDe").val() == "" || !fetch.test($("#dtDe").val()) ){
            $("#dtDe").focus().after("<span class='error'>Ingrese la fecha</span>");
            return false;
		...
		$.ajax({
			type:'POST',
			url: '../Code_Menu/LlenaDatosComprasProveedor.php',
	...
	//Evento del Boton de Consultar del Vistas Detallada .- Profundidad de Llantas
	...
	//Evento del Boton Consultar de Marcas de llantas
	...
	$("#ConsultarLlantaEstatusConsumo").click(function () { 
		...
		var formatter = new google.visualization.NumberFormat({prefix: "$"});
  		...
  		new google.visualization.Dashboard(document.getElementById('dashboard2_div')).
